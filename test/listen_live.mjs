...
const result = await page.evaluate(() => {
  const registry = {
    ...
    sources: [
      {
        id: 'home-radio', sport: 'nfl', scope: 'home', language: 'en',
        name: 'Home Radio', provider: 'Official Station', access: 'free',
        providerUrl: 'https://radio.example/listen',
        streamUrl: 'https://radio.example/live.mp3', directPlaybackAllowed: true
      },
      {
        id: 'national-pass', sport: 'nfl', scope: 'national', language: 'es',
        name: 'National Audio', provider: 'League Audio', access: 'subscription',
        providerUrl: 'https://league.example/audio', directPlaybackAllowed: false,
        notes: 'Authentication handled by provider.'
      },
      {
        id: 'unsafe-stream', sport: 'nfl', scope: 'away', language: 'en',
        name: 'Unsafe Direct', access: 'free', providerUrl: 'https://away.example',
        streamUrl: 'https://away.example/live.mp3', directPlaybackAllowed: false
      }
    ]
  };
  const mappings = { schemaVersion: 1, games: { 'game-1': { sources: ['national-pass', 'home-radio', 'unsafe-stream'] } } };
  const rendered = window.GamecastListenLive.render({
    container: document.getElementById('listenLiveBox'), eventId: 'game-1', registry, mappings
  });
...
assert('sorts home before away/national', result.names[0] === 'Home Radio', result.names);
...
const empty = await page.evaluate(() => {
  window.GamecastListenLive.render({
    container: document.getElementById('listenLiveBox'), eventId: 'missing',
    registry: { schemaVersion: 1, sources: [] }, mappings: { schemaVersion: 1, games: {} }
  });
