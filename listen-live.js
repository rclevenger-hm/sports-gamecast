(function (root) {
  ...
  function injectStyles() {
    ...
    style.textContent = [
      ...
      ".listen-live-sub,.listen-live-empty,.listen-note,.listen-warning{color:var(--muted)}",
      ".listen-live-sub{font-size:11px}.listen-live-empty{font-size:12px;line-height:1.45}",
      ".listen-source{padding:10px 0;border-top:1px solid var(--border)}.listen-source:first-of-type{border-top:0}",
      ...
      ".listen-note{font-size:11px;margin-top:5px;line-height:1.4}",
      ...
      ".listen-link:hover{border-color:var(--accent);color:var(--accent)}.listen-audio{width:min(100%,420px);height:34px}.listen-warning{font-size:10px}"
    ].join("");
  ...
  function languageLabel(code) { return LANGUAGE_NAMES[code] || (code ? code.toUpperCase() : "Language n/a"); }
  ...
  function sourceNote(source) {
    var parts = [], access = accessLabel(source);
  ...
  function resolveSources(eventId, registry, mappings) {
    ...
    var byId = new Map((registry && Array.isArray(registry.sources) ? registry.sources : []).map(function (source) { return [source.id, source]; }));
    return refs.map(function (ref) {
      var id = typeof ref === "string" ? ref : ref && ref.id;
      var source = byId.get(id);
      if (!source) return null;
      var effectiveScope = typeof ref === "object" && ref.scope ? ref.scope : source.scope;
      return Object.assign({}, source, { scope: effectiveScope });
  ...
  function renderSource(source) {
    var item = make("div", "listen-source"), row = make("div", "listen-source-row");
    ...
    if (streamUrl) {
      ...
      audio.className = "listen-audio"; audio.controls = true; audio.preload = "none"; audio.src = streamUrl;
    ...
    if (providerUrl) {
      ...
      link.href = providerUrl; link.target = "_blank"; link.rel = "noopener noreferrer"; actions.appendChild(link);
    }
    if (!streamUrl && !providerUrl) actions.appendChild(make("span", "listen-warning", "No usable authorized listening link is currently available."));
  ...
  function render(options) {
    ...
    if (!host) return null;
    host.innerHTML = ""; host.className = "listen-live"; host.setAttribute("aria-live", "polite");
  ...
  async function mount() {
    var gameUi = document.getElementById("gameUI"), situation = document.getElementById("situationBox");
    if (!gameUi || !situation) return;
    var eventId = new URLSearchParams(location.search).get("event");
    if (!eventId) return;
    var box = document.getElementById("listenLiveBox");
    if (!box) { box = document.createElement("section"); box.id = "listenLiveBox"; situation.insertAdjacentElement("afterend", box); }
    box.className = "listen-live"; box.textContent = "Loading authorized audio sources…";
    try {
      ...
      injectStyles(); box.innerHTML = "";
      var head = make("div", "listen-live-head"); head.appendChild(make("div", "listen-live-title", "Listen Live")); box.appendChild(head);
